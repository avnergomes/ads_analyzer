...
class IntegratedDashboard:
    ...
    def render_show_health_indicators(
        ...
        avg_sales_7d = avg_sales_last_7_days
    ...
    def render_show_health(
        ...
        sales_to_date = safe_numeric_local(sales_to_date)
        avg_ticket_price = safe_numeric_local(avg_ticket_price)
        days_to_show = safe_numeric_local(days_to_show)
        avg_sales_last_7_days = safe_numeric_local(avg_sales_last_7_days)
        ...
        with col3:
            st.metric(
                "Revenue",
                f"${sales_to_date:,.0f}",
                delta=f"${avg_ticket_price:.0f} avg ticket"
            )
            
        with col4:
            daily_target = latest.get('remaining', 0) / max(days_to_show, 1)
            st.metric(
                "Daily Target",
                f"{daily_target:.0f}",
                delta=f"{avg_sales_last_7_days:.0f} avg 7d"
            )
        ...
        with col2:
            if budget_input > 0 and latest.get('total_sold', 0) > 0:
                ...
                with col2_2:
                    if funnel and funnel.spend > 0:
                        daily_cpa = funnel.spend / total_sold
            ...
            if len(show_records) >= 7:
                ...
                fig.add_trace(go.Bar(
                    ...
                    marker_color=recent["today_sold"].apply(
                        lambda x: "#27ae60" if safe_numeric_local(x) > daily_target else "#e67e22" if x > daily_target*0.5 else "#e74c3c"
                    ),
